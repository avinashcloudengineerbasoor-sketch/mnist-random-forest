...
def train():
    ...
    final_model = FinalClassifier(metadata_dim=metadata_dim).to(device)
    ...
    for i in range(0, len(X_img), batch_size):
        ...
        batch_meta = X_meta[i:i + batch_size].to(device)
        ...
        if i % 20000 == 0:
            print(
                f"Training step {i}, loss={loss.item():.4f}"
            )

    print("Saving model artifacts...")

    torch.save(image_model.state_dict(), "artifacts/image_model.pth")
    torch.save(final_model.state_dict(), "artifacts/final_classifier.pth")

    joblib.dump(encoder, "artifacts/metadata_encoder.joblib")
    ...
    print("✅ Training complete. Model + encoder + example files saved.")
